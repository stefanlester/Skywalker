...
func doMake(arg2, arg3 string) error {
	...
	case "handler":
		...
		fileName := sky.RootPath + "/handlers/" + strings.ToLower(arg3) + ".go"
	...
	case "model":
		...
		fileName := sky.RootPath + "/data/" + strings.ToLower(modelName) + ".go"
		if fileExists(fileName) {
			exitGracefully(errors.New(fileName + " already exists!"))
		}
	...
	case "mail":
		...
		htmlMail := sky.RootPath + "/mail/" + strings.ToLower(arg3) + ".html.tmpl"
		plainMail := sky.RootPath + "/mail/" + strings.ToLower(arg3) + ".plain.tmpl"
